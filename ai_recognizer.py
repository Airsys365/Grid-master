...
АНАЛИЗ ОБРАМЛЕНИЯ И КИКПЛЕЙТА:
Внимательно изучи выноски, обозначения и размеры на всех кромках изделия.
На одном изделии могут быть разные типы обрамления на разных сторонах.
Это может быть обозначено по-разному: "Tun A/B/D", "тип 1/2", "frame/kickplate", стрелками с подписями, таблицей материалов.
Признаки кикплейта/отбойника: полоса вдоль кромки с указанием ширины (например 150мм), более тяжёлый профиль, буртик.
Признаки стандартного обрамления: тонкий уголок или полоса по периметру без специального указания ширины.

Для каждой позиции определи:
- frame_mm: суммарная длина СТАНДАРТНОГО обрамления по всем сторонам где оно есть (мм). null если не ясно (будет посчитано автоматически).
- bumper_mm: суммарная длина КИКПЛЕЙТА/ОТБОЙНИКА по всем сторонам где он есть (мм). 0 или null если кикплейта нет.
- frame_analysis: краткое текстовое описание твоего вывода (какой тип на какой стороне и почему). Всегда заполняй если видишь выноски с типами.

Если не видишь разных типов обрамления — оставь frame_mm и bumper_mm null (программа посчитает периметр сама).
...
Особо внимательно изучи:
1. Все выноски и подписи на кромках изделий — они могут указывать тип обрамления или кикплейта
2. Размеры рядом с кромками — ширина профиля, длина участка
3. Спецификацию или таблицу материалов если есть на чертеже
4. Различные штриховки или типы линий на разных кромках
...
[
  {
    ...
    "cutout_arc_fraction": null,
    "frame_mm": 1093,
    "bumper_mm": 1087,
    "frame_analysis": "Tun A (стандартное обрамление) на верхней стороне 690мм и нижней правой части 403мм = 1093мм. Tun D (кикплейт 150мм) на левой стороне 400мм + правой стороне 400мм + нижней левой части 287мм = 1087мм.",
    "notes": "два типа обрамления на разных кромках",
    "confidence": "high"
  }
]

Если разных типов обрамления нет — ставь frame_mm: null, bumper_mm: null, frame_analysis: null.
"""
...
def _normalize_part(p: Dict) -> Dict:
    ...
    # frame/bumper overrides из анализа ИИ
    frame_mm = p.get("frame_mm")
    bumper_mm = p.get("bumper_mm")
    try:
        frame_override = float(frame_mm) if frame_mm is not None else 0.0
    except Exception:
        frame_override = 0.0
    try:
        bumper_override = float(bumper_mm) if bumper_mm is not None else 0.0
    except Exception:
        bumper_override = 0.0

    frame_analysis = str(p.get("frame_analysis") or "")

    # Если ИИ определил frame_analysis, добавить его к notes для отображения
    notes = str(p.get("notes") or "")
    if frame_analysis and frame_analysis not in notes:
    ...
    return {
        ...
        "bumper_override":    bumper_override,
