...
def _default_config() -> Dict:
    return {
        ...
        "labour_norms": {
            ...
            # ~5 мин/м = 0.083 ч/м — реалистично для ручного реза решётки
            "t_straight_h_per_m":   0.083,
            # доп. время в часах на один косой рез (трапеция)
            # ~15 мин за один косой рез разметка+рез+зачистка
            "t_diagonal_h_per_cut": 0.25,
            # время в часах на погонный метр дуги радиуса (ручная вырезка)
            # ~20 мин/м — ручная резка по дуге значительно сложнее прямого реза
            "t_arc_h_per_m":        0.33,
            # коэфф. сложности: чем меньше площадь, тем больше накладных расходов
            # complexity = max(1.0, k / sqrt(area_m2))
            "complexity_k":         0.30,
            # минимум часов на любую деталь (независимо от размера)
            "min_hours_per_part":   0.15,
        },
    }
